...
file_path = r"C:\Users\VAQ\OneDrive - TRƯỜNG ĐẠI HỌC MỞ TP.HCM\University\Research paper\RP_2 ngươi bạn\Data\ML Process\Data_csv\rawdata_1.csv"
...
class ICTIntellectualCapitalAnalysis:
    ...
    def process_data(self):
        ...
        # 1. Lọc năm từ 2010–2024
        df_work = df_work[(df_work["Time"] >= 2010) & (df_work["Time"] <= 2024)]
        print(f"Số dòng sau khi lọc năm 2010–2024: {len(df_work)}")

        # 2. Drop toàn bộ các dòng có missing value
        null_count = df_work.isnull().sum().sum()
        print(f"Số giá trị thiếu trước khi drop: {null_count}")
        df_work = df_work.dropna()
        print(f"Số dòng sau khi drop missing: {df_work.shape}")
